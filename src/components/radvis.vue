  ...
  .section
    ...
    .graph-field
        ...
        .correlation-field
          ...
          template(v-for="dimension in dimensions")
            .correlation-group(v-if="dimension.usage",v-bind:style="{ height : (264 / getActiveDimensionSize()) + 'px'}")
              .name-vertical
                .name {{dimension.name}}
              template(v-for="(corrValue,k) in dimension.correlation")
                .correlation-block(v-if="getDimensionByName(k).usage",v-bind:style="{ background : getCorrelation(corrValue) }")
      .svg-group
        svg.radvis
          g.gBackground(transform='translate(540,540)')
            circle(cx="0", cy="0", r="400", fill="none", stroke="#333", stroke-dasharray="3,6")
          g.gDimensions(transform='translate(540,540)')
            template(v-for="dimension in dimensions")
          ...
          g.gNodes(transform='translate(540,540)')
      ...
      .side-view
        ...
        .flex-group.debug
          .flex-name Dimension Font Size
          .flex-text : {{dimensionFontSize}}
        .category Clustering
